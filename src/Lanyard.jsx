...
function Band({ maxSpeed = 50, minSpeed = 0 }) {
  ...
  // --- LOGIKA FISIKA TALI (JOINTS) ---
  useRopeJoint(fixed, j1, [[0, 0, 0], [0, 0, 0], 0.6]); 
  useRopeJoint(j1, j2, [[0, 0, 0], [0, 0, 0], 0.6]);
  useRopeJoint(j2, j3, [[0, 0, 0], [0, 0, 0], 0.6]);
  
  // Joint Kartu (PENTING: Koordinat ini menentukan posisi gantung kartu)
  useSphericalJoint(j3, card, [
    [0, 0, 0], 
    [0, 1.45, 0] // Anchor point di atas tengah kartu
  ]);
    ...
    <>
      <group position={[0, 4, 0]}>
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          <CuboidCollider args={[0.8, 1.125, 0.1]} />
          <group
            scale={2.25}
            position={[0, -1.2, -0.05]}
